...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="rounded-2xl border border-app-border bg-surface p-8">
            ...
            <form method="POST" action="{{ route('trainer.settings.update') }}" class="space-y-5"
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-300">Bio</label>
                    <textarea name="bio" rows="4"
                        class="w-full rounded-xl border border-app-border bg-surface px-4 py-3 text-white focus:border-brand">{{ old('bio', $profile->bio) }}</textarea>
                    @error('bio')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-300">Tags</label>
                    <input type="text" name="tags" value="{{ old('tags', implode(', ', $profile->tags ?? [])) }}"
                        class="w-full rounded-xl border border-app-border bg-surface px-4 py-3 text-white focus:border-brand"
