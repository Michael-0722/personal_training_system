...
@section('content')
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-surface rounded-2xl border border-app-border p-6">
            ...
            <p class="text-2xl font-bold font-heading text-brand text-right">₱{{ number_format($totalEarnings, 2) }}</p>
        </div>
        <div class="bg-surface rounded-2xl border border-app-border p-6">
            <p class="text-xs text-gray-500 uppercase mb-1">Pending</p>
            <p class="text-2xl font-bold font-heading text-yellow-400 text-right">₱{{ number_format($pendingPayouts, 2) }}</p>
        </div>
        <div class="bg-surface rounded-2xl border border-app-border p-6">
            <p class="text-xs text-gray-500 uppercase mb-1">Paid Out</p>
            <p class="text-2xl font-bold font-heading text-blue-400 text-right">₱{{ number_format($paidPayouts, 2) }}</p>
        </div>
        <div class="bg-surface rounded-2xl border border-app-border p-6">
            <p class="text-xs text-gray-500 uppercase mb-1">Commission</p>
            <p class="text-2xl font-bold font-heading text-red-400 text-right">₱{{ number_format($totalCommission, 2) }}</p>
        </div>
    </div>
    <div id="trainer-earnings" class="bg-surface rounded-2xl border border-app-border overflow-hidden">
    ...
    <div class="mt-6">{{ $transactions->fragment('trainer-earnings')->links() }}</div>
@endsection
